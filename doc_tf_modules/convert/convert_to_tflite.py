...
def convert_to_tflite(saved_model_dir, tflite_model_path):
    """
    Convert TensorFlow SavedModel to TFLite format optimized for mobile/embedded devices
    ...
    # Enable quantization for smaller model size and better performance
    ...
    # Set inference input/output types
    converter.inference_input_type = tf.float32
    converter.inference_output_type = tf.float32
    
    # Convert the model
    print("Converting model to TFLite format (optimized for mobile/embedded)...")
...
def create_tflite_inference_script():
    '''
    Creates a Python script for running inference with the TFLite model
    '''
    script_path = 'tflite_inference.py'
...
def preprocess_patch(patch):
    ...
    # Add batch dimension and ensure channel-last format (NHWC)
...
''')
    ...
    print(f"TFLite inference script created at 'tflite_inference.py'")
    print("Run with: python tflite_inference.py --input <input_image> --output <output_image> --model docres_model.tflite")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tflite_model_path', default='docres_model.tflite', help='Output TFLite model path')
    
    args = parser.parse_args()
    
    tflite_model_path = convert_to_tflite(args.saved_model_dir, args.tflite_model_path) 